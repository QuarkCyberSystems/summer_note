...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["parent", "customer", "customer_group", "posting_date","item_code", "item_name","item_group", "brand", "description", \
			...
			"buying_amount", "gross_profit", "gross_profit_percent", "project", "sales_person", "sales_order","income_account","expense_account", "source"],
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		#"source": _("Source") + "Data:100",
		"sales_order": _("Sales Order") + ":Link/Sales Order:100",
		"income_account": _("Income Account") + ":Link/Account:100",
		"expense_account": _("Expense Account") + ":Link/Account:100",
		"source": _("Source") + ":Data:100",
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice Item`.warehouse, `tabSales Invoice Item`.item_group,`tabSales Invoice Item`.sales_order,
				`tabSales Invoice Item`.brand, `tabSales Invoice Item`.dn_detail,`tabSales Invoice Item`.income_account,`tabSales Invoice Item`.expense_account,
			...
			.format(conditions=conditions, sales_person_cols=sales_person_cols,
				sales_team_table=sales_team_table, 
				 match_cond = get_match_cond('Sales Invoice')), self.filters, as_dict=1)

		#frappe.errprint(self.si_list)
		#for item in self.si_list:
		for i,x in enumerate(self.si_list):
			qt = frappe.get_value("Sales Order Item", {"parent":x.sales_order}, "prevdoc_docname")
			self.si_list[i]['source'] = frappe.get_value("Quotation", qt, "source")
		#frappe.errprint(self.si_list)		 
