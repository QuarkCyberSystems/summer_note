...
def inter_company(start_date, end_date, payroll_entry):
    ...
    frappe.msgprint("Inter Company WPS JVs have been set to darft")
        ...
        if leave_application.leave_type != "Sick Leave" and leave_application.leave_type != "Leave Without Pay":
            ...
            if (arcal) > 0:
                ...
                if (arcal) > 0:
                    ...
                    jv.append("accounts",{
                            "account": account_ledger,
                            "credit_in_account_currency": arcal,
                            "cost_center":frappe.get_value("Department", frappe.get_value("Employee", item.employee, "department"), "cost_center")
                            })
                    jv.leave_application = leave_application.name
                    jv.save(ignore_permissions=True)
...
def add_benefits(salary_slip, method):
    ...
    if emp.eosb == 0:
        ...
        if frappe.get_value("Department", frappe.get_value("Employee", salary_slip.employee, "department"), "eosb_receivable") == 1:
            jv.append("accounts",{
                    "account":eosb_ledger,
                    "credit_in_account_currency": post_eosb,
                    "cost_center" : nest_cc
    ...
    if salary_slip.company != s_comp:
        ...
        jv.append("accounts",{
                    ...
                    "cost_center":frappe.get_value("Department", frappe.get_value("Employee", salary_slip.eemployee, "department"), "cost_center")
                    })
        jv.salary_slip.name            
        jv.save()
